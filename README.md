...
- **PythonNative Go dev client:** Install a prebuilt client once with `pn go install`, then `pn start` serves your `app/` over Wi-Fi (with a QR code) and Fast-Refreshes the running app on every save, preserving component state across most changes. It's the Python answer to Expo Go: one install runs any project, no native rebuild in the loop.
