...
from ..dev import DEV_CLIENT_ENTRY
...
class Builder:
    ...
    def prepare(self, platform: str, *, dev_client: bool = False) -> PreparedProject:
        """Stage and configure the native project for ``platform``.

        Args:
            platform: ``"android"`` or ``"ios"``.
            dev_client: When ``True``, build the **PythonNative Go** client:
                the user's ``app/`` is not bundled, the root entry is the
                dev-client shell, and LAN cleartext networking is enabled so
                the app can reach a ``pn start`` dev server.
        ...
        if platform == "android":
            ...
            self._write_entry_module(layout.python_root, dev_client=dev_client)
        ...
        ios_layout = ios_config.configure(project_dir, self.config, log=self.log, dev_client=dev_client)
        # iOS Python sources are staged into a side directory and embedded
        # into the built .app after the build.
        python_dir = self._stage_ios_python(build_dir, dev_client=dev_client)
        self._write_entry_module(python_dir, dev_client=dev_client)
        return PreparedProject(
            platform=platform,
            build_dir=build_dir,
            project_dir=project_dir,
    ...
    def _write_entry_module(self, python_root: Path, *, dev_client: bool) -> None:
        """Write the generated ``pn_entry`` marker read by ``pythonnative.bootstrap``."""
        entry = DEV_CLIENT_ENTRY if dev_client else self.config.entry_module
        content = (
            "# Generated by the `pn` CLI. Tells pythonnative.bootstrap which\n"
            "# root component the native templates should mount.\n"
            f"ENTRY_MODULE = {entry!r}\n"
            f"DEV_CLIENT = {bool(dev_client)!r}\n"
        )
        python_root.mkdir(parents=True, exist_ok=True)
        (python_root / "pn_entry.py").write_text(content, encoding="utf-8")

    def _stage_ios_python(self, build_dir: Path, *, dev_client: bool = False) -> Path:
        import shutil
