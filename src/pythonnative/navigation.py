...
class _DeclarativeNavHandle:
    ...
    def go_back(self) -> None:
        ...
        self._set_stack(lambda s: list(s[:-1]) if len(s) > 1 else list(s))
...
class _TabNavHandle(_DeclarativeNavHandle):
    ...
    def __init__(
        ...
        super().__init__(screen_map, get_stack, set_stack)
...
class _DrawerNavHandle(_DeclarativeNavHandle):
    ...
    def navigate(self, route_name: str, params: Optional[Dict[str, Any]] = None) -> None:
        """Switch to a screen and close the drawer."""
        if route_name not in self._screen_map:
            raise ValueError(f"Unknown route: {route_name!r}. Known routes: {list(self._screen_map)}")
...
def _stack_navigator_impl(screens: Any = None, initial_route: Optional[str] = None) -> Element:
    ...
    handle = use_memo(lambda: _DeclarativeNavHandle(screen_map, lambda: stack_ref["current"], set_stack), [])
...
def _tab_navigator_impl(screens: Any = None, initial_route: Optional[str] = None) -> Element:
    ...
    handle = use_memo(lambda: _TabNavHandle(screen_map, get_stack, lambda _: None, switch_tab), [])
    handle._screen_map = screen_map
    ...
    for s in screen_list:
        if not isinstance(s, _ScreenDef):
            continue
        is_active = s.name == active_tab
        label = s.options.get("title", s.name)
        tab_name = s.name

        def make_on_click(n: str) -> Callable[[], None]:
            return lambda: switch_tab(n)
        ...
        tab_buttons.append(
            Element(
                ...
                {
                    "title": label,
                    "on_click": make_on_click(tab_name),
                    "enabled": not is_active,
                },
                [],
                key=f"__tab_{tab_name}",
            )
        )

    tab_bar = Element(
        "View",
        {"flex_direction": "row", "background_color": "#F8F8F8"},
        tab_buttons,
    )
